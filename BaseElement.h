...
namespace ui {
    ...
    public:
		...
		const math::vec2 &rel_pos() const { return rel_dims_[0]; }
		const math::vec2 &rel_size() const { return rel_dims_[1]; }

        const math::vec2 &pos() const { return dims_[0]; }
        const math::vec2 &size() const { return dims_[1]; }

        const math::ivec2 &pos_px() const { return dims_px_[0]; }
        const math::ivec2 &size_px() const { return dims_px_[1]; }
    ...
    public:
    RootElement(const math::ivec2 &zone_size) : BaseElement({-1, -1}, {2, 2}, nullptr) {
            set_zone(zone_size);
